...
export const AddArticleForm = memo((props: AddArticleProps) => {
	...
	const getEmptyBlock = useCallback((blocks?: ArticleBlock[]) => {
		if (!blocks) {
			return {
				id: '1',
				order: 1
	...
	const onChangeBlockState = (editedBlock: ArticleBlock) => {
		...
		onChangeBlocks(data.blocks.map(block => {
	...
	const dropHandler = (e: DragEvent<HTMLDivElement>, editBlock: ArticleBlock) => {
		e.preventDefault();

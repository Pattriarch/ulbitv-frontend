...
export const EditArticleImageBlock = memo((props: EditArticleImageBlockProps) => {
	const {
		className,
		block,
		onChangeBlockState,
	...
	if (isEdit) {
		return (
			<div
				...
				{...otherProps}
			>
				<MarkIcon onClick={onClickMarkIcon} className={classNames(cls.icon, {}, [cls.markIcon])}/>
				<CancelIcon onClick={onClickCancelIcon} className={classNames(cls.icon, {}, [cls.cancelIcon])}/>
				<img src={imageBlock.src} className={cls.preview} alt={imageBlock.title}/>
				<Text text={'Путь к изображению'}/>
				<Input theme={'outlined'} value={imageBlock?.src} onChange={onChangeImage}/>
				<Text text={'Текст под изображенением'}/>
				<Input theme={'outlined'} value={imageBlock?.title} onChange={onChangeTitle}/>
			</div>
		);
	}

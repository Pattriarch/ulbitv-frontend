...
export const ArticleCard = memo((props: ArticleCardProps) => {
	...
	if (error) {
		return (
			<div className={classNames(cls.EditArticleForm, {}, [className])}>
				<Text theme={TextTheme.ERROR} title={t('Произошла ошибка при загрузке статьи')}/>
			</div>
		);
	}
	...
	return (
		<div className={classNames(cls.ArticleCard, {}, [className])}>
			...
			<Input theme={'outlined'} className={cls.title} value={data?.title} onChange={onChangeTitle}/>
			<Text text={'Подзаголовок статьи'}/>
			<Input theme={'outlined'} className={cls.subtitle} value={data?.subtitle} onChange={onChangeSubtitle}/>
			<div className={cls.avatarWrapper}>
				<Avatar size={200} src={data?.img} className={cls.avatar}/>
			</div>
			<Text text={'Путь к изображению'}/>
			<Input theme={'outlined'} className={cls.subtitle} value={data?.img} onChange={onChangeImage}/>
			<Text text={'Содержимое статьи'}/>
			{data?.blocks?.slice()?.sort(sortBlocks)?.map((block) => (
			...
			<div className={cls.actions}>
				...
				<Button onClick={onAddImageBlock}>{t('Добавить изображение')}</Button>
			</div>
		</div>
	);
});

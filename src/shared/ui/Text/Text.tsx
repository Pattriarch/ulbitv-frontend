...
export const Text = memo((props: TextProps): JSX.Element => {
  ...
  const getTitle = useCallback(() => {
    switch (tag) {
      ...
      case "h1":
        return (
          <h1 className={cls.title} data-testid={`${dataTestId}.Header`}>
            {title}
          </h1>
        );
      case "h2":
        return (
          <h2 className={cls.title} data-testid={`${dataTestId}.Header`}>
            {title}
          </h2>
        );
      case "h3":
        return (
          <h3 className={cls.title} data-testid={`${dataTestId}.Header`}>
            {title}
          </h3>
        );
    }
